...
fn main() {
    ...
    let times = input[0].split_once(":").unwrap().1
        .trim()
        .split(" ")
        .filter(|s| !s.is_empty())
    ...
    let distances = input[1].split_once(":").unwrap().1
        .trim()
        .split(" ")
        .filter(|s| !s.is_empty())
        .map(|s| s.trim().parse::<u64>().unwrap())
    ...
    for (time, distance) in times.iter().zip(distances.iter()) {
        ...
        println!("ways: {}", calc_ways_to_win(*time, *distance));
        res *= calc_ways_to_win(*time, *distance);
    }
